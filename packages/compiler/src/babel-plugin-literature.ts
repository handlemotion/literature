...
import { buildTextTarget } from "@literature/core";
import type { TextTarget } from "@literature/core";
      ...
      Program: {
        exit(programPath) {
          ...
          const hasImport = programPath.node.body.some(
            (n) => t.isImportDeclaration(n) && n.source.value === "@literature/client/lit",
          );
          if (hasImport) return;
          const importDecl = t.importDeclaration(
            [t.importSpecifier(t.identifier("__lit"), t.identifier("__lit"))],
            t.stringLiteral("@literature/client/lit"),
          );
